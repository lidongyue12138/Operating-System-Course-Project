...
int lsh_history(char **args)
{
    ...
    if (strcmp(args[0], "history") == 0){ /* Print out history commands */
...
int lsh_redirect_output(char **args)
{
    pid_t pid, wpid;
    int status;
    ...
    pid = fork();
    if(pid == 0){ /* child process */
        char ** run_arg = malloc(LSH_TOK_BUFSIZE * sizeof(char*));
        int j=0;
        while (true)
        {
...
int lsh_redirect_input(char **args)
{
    pid_t pid, wpid;
    int status;
...
int lsh_pipe(char **args)
{
    ...
    pid = fork();
    if(pid == 0){ /* child process */
        char **pipeHeadArgs = malloc(LSH_TOK_BUFSIZE * sizeof(char*));
        char **pipeTailArgs = malloc(LSH_TOK_BUFSIZE * sizeof(char*));
        int j=0; int tmpNum;
        while (true)
        {
            if (strcmp(args[j],"|")==0) {tmpNum = ++j;break;}
            pipeHeadArgs[j] = args[j];
            j++;
        }
        pipeHeadArgs[j] = NULL;
        while (true)
        {
            ...
            pipeTailArgs[j-tmpNum] = args[j];
            j++;
        }
        pipeTailArgs[j-tmpNum+1] = NULL;
        ...
        {   // replace standard input with input part of pipe
            ...
            execvp(pipeTailArgs[0], pipeTailArgs);
        }
        else
            {
            ...
            execvp(pipeHeadArgs[0], pipeHeadArgs);
        }
    }
    else if(pid > 0){ /* parent process */
        if(!conc){
...
int lsh_execute(char *line){
    ...
    {
        if (strcmp(args[j], ">") == 0)
            return lsh_redirect_output(args);
        j++;
        if (args[j] == NULL) break;
    }
    ...
    while (true)
    {
        if (strcmp(args[j], "<") == 0)
        {
            return lsh_redirect_input(args);
        }
        j++;
        if (args[j] == NULL) break;
    }
