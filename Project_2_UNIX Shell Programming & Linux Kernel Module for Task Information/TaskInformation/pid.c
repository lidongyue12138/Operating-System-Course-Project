...
static ssize_t proc_read(struct file *file, char __user *usr_buf, size_t count, loff_t *pos)
{
        ...
        if (tsk==NULL) rv = sprintf(buffer, "command = [no such task] pid = [%ld] state=[no such task]", l_pid);
	else rv = sprintf(buffer, "command = [%s] pid = [%ld] state=[%ld]", tsk->comm, l_pid, tsk->state);
        completed = 1;
